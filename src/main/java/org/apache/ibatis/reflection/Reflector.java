...
public class Reflector {
    ...
    private void addDefaultConstructor(Class<?> clazz) {
        ...
        for (Constructor<?> constructor : consts) {
            if (constructor.getParameterTypes().length == 0) { //默认构造方法没有参数
                if (canAccessPrivateMethods()) { //允许利用反射检查任意类的私有方法
                    try {
        ...
        for (Method method : methods) {
            ...
            if (name.startsWith("get") && name.length() > 3) {
                if (method.getParameterTypes().length == 0) {
                    name = PropertyNamer.methodToProperty(name); //获取字段名
    ...
    private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
        for (String propName : conflictingGetters.keySet()) { //遍历conflictingGetters集合
            List<Method> getters = conflictingGetters.get(propName); // 根据属性名称获取对应的方法
    ...
    private void addUniqueMethods(Map<String, Method> uniqueMethods, Method[] methods) {
        for (Method currentMethod : methods) {
            if (!currentMethod.isBridge()) {
                ...
                // overridden a method
                if (!uniqueMethods.containsKey(signature)) { //根据方法签名排重
                    if (canAccessPrivateMethods()) {
                        try {
    ...
    private static boolean canAccessPrivateMethods() { // 获取访问私有方法的权限
        try {
